...
# Convert attack category to binary (Shellcode vs. others)
df['is_Shellcode'] = df['attack_cat'].apply(lambda x: 1 if x == 'Shellcode' else 0)
...
X = np.array(ct.fit_transform(df.drop(columns=['is_Shellcode'])))
y = df['is_Shellcode']
