...
# Convert attack category to binary (Worms vs. others)
df['is_Worms'] = df['attack_cat'].apply(lambda x: 1 if x == 'Worms' else 0)
...
X = np.array(ct.fit_transform(df.drop(columns=['is_Worms'])))
y = df['is_Worms']
