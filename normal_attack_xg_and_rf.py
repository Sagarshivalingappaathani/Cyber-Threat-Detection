...
# Convert attack category to binary (Normal vs. others)
df['is_Normal'] = df['attack_cat'].apply(lambda x: 1 if x == 'Normal' else 0)
...
X = np.array(ct.fit_transform(df.drop(columns=['is_Normal'])))
y = df['is_Normal']
