...
# Convert attack category to binary (Fuzzers vs. others)
df['is_Fuzzers'] = df['attack_cat'].apply(lambda x: 1 if x == 'Fuzzers' else 0)
...
X = np.array(ct.fit_transform(df.drop(columns=['is_Fuzzers'])))
y = df['is_Fuzzers']
