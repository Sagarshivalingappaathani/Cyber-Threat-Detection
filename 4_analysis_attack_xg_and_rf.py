...
# Convert attack category to binary (Analysis vs. others)
df['is_Analysis'] = df['attack_cat'].apply(lambda x: 1 if x == 'Analysis' else 0)
...
X = np.array(ct.fit_transform(df.drop(columns=['is_Analysis'])))
y = df['is_Analysis']
