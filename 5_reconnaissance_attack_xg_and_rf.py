...
# Convert attack category to binary (Reconnaissance vs. others)
df['is_Reconnaissance'] = df['attack_cat'].apply(lambda x: 1 if x == 'Reconnaissance' else 0)
...
for feature in df_numeric.columns:
    if df_numeric[feature].nunique() > 50:
        # Apply log transformation. Adjust to avoid taking log of non-positive numbers if necessary.
...
ct = ColumnTransformer(
    transformers=[('encoder', OneHotEncoder(handle_unknown='ignore'), df.select_dtypes(exclude=[np.number]).columns)],
    remainder='passthrough'
)
X = np.array(ct.fit_transform(df.drop(columns=['is_Reconnaissance'])))
y = df['is_Reconnaissance']
